...
esp_err_t lcd_gc9a01_init(const scr_controller_config_t *lcd_conf)
{
    ...
    //init back light with 0 when io 0 is 
    if (lcd_conf->pin_num_bckl == DOWNLOAD_PIN)
    {
...
{
    esp_err_t ret;
    uint8_t data[] = {0x48, 0xC8, 0x68, 0xA8};
    ...
    ESP_LOGI(TAG, "MADCTL=%x", data[dir]);
    ret = LCD_WRITE_REG(LCD_MADCTL, data[dir]);
